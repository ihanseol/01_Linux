...
// 2005.09.15
// 2006.02.23 - uses the arcs with specified (minimum) diameter
// 2008.09.25 - uses simple bezier curve again / tested with CS3
// Copyright(c) 2008 SATO Hiroyuki
...
function main(){
  ...
  var sp = extractPathes(s.slice(0),1);
  ...
  for(var i=sp.length-1; i>=1; i--){
    for(j=i-1; j>=0; j--){
      pi = metaball(sp[i], sp[j], v, handle_len_rate);
      if(pi != null) sp2.push(pi);
    }
  }
  app.activeDocument.selection = sp2;
}
...
// and return an Array of PathItems.  If "pp_length_limit" is specified,
...
function extractPathes(s, pp_length_limit){
  ...
  for(var i=0; i<s.length; i++){
    if(s[i].typename=="PathItem"
        // ignore pathes for clipping path and guide
